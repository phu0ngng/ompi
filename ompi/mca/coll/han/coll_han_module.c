...
static void mca_coll_han_module_construct(mca_coll_han_module_t * module)
{
    ...
    module->cached_low_comms_dup = NULL;
...
mca_coll_han_module_destruct(mca_coll_han_module_t * module)
{
    ...
    if (module->cached_low_comms_dup != NULL) {
        for (i = 0; i < COLL_HAN_LOW_MODULES; i++) {
            ompi_comm_free(&(module->cached_low_comms_dup[i]));
            module->cached_low_comms_dup[i] = NULL;
        }
        free(module->cached_low_comms_dup);
        module->cached_low_comms_dup = NULL;
    }
