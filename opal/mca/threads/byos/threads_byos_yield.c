...
static void opal_thread_byos_yield_sched_yield(void);
static void opal_thread_byos_yield_nanosleep(void);

typedef enum {
    OPAL_PTHREADS_YIELD_SCHED_YIELD = 0,
    OPAL_PTHREADS_YIELD_NANOSLEEP
} opal_threads_byos_yield_strategy_t;
...
static opal_threads_byos_yield_strategy_t yield_strategy = OPAL_PTHREADS_YIELD_SCHED_YIELD;

opal_threads_byos_yield_fn_t *opal_threads_byos_yield_fn
    = &opal_thread_byos_yield_sched_yield;

int opal_threads_byos_yield_init(const mca_base_component_t *component)
{
    ...
    (void) mca_base_component_var_register(component, "yield_strategy",
                                           "Yield strategy to use if none is provided by the application",
    ...
    case OPAL_PTHREADS_YIELD_NANOSLEEP:
        opal_threads_byos_yield_fn = &opal_thread_byos_yield_nanosleep;
        break;
    default:
        /* use initial value */
        break;
...
void opal_thread_byos_yield_sched_yield(void)
{
#ifdef HAVE_SCHED_H
    sched_yield();
#endif
...
void opal_thread_byos_yield_nanosleep(void)
{
